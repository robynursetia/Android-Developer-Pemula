...
public class RobbyData {
    ...
    public static ArrayList<Robby> getListData(){
        Robby robby = null;
        ArrayList<Robby> list = new ArrayList<>();
        for (int i = 0; i<data.length; i++){
            robby = new Robby();
            robby.setName(data[i][0]);
            robby.setRemarks(data[i][1]);
            robby.setPhoto(data[i][2]);
            robby.setDeskripsi(data[i][3]);
            robby.setLahir(data[i][4]);
            robby.setWafat(data[i][5]);
